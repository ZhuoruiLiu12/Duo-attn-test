...
def parse_args():
    ...
    parser.add_argument(
        "--data-path",
        type=str,
        default="/disk12/liuzhuorui/datasets/longbench/LongBench.py"
    )
    parser.add_argument(
        "--iter",
        type=int,
...
def main(args):
    ...
    dataset = load_dataset(args.data_path, "qasper", split="test", trust_remote_code=True)
    # warmup
    for _ in range(10):
    ...
    for _ in range(args.iter):
        ...
        item = next(iterators)
        input_ids = tokenizer(item['context'], return_tensors='pt').input_ids.to(model.device)
        original_length = input_ids.shape[1]
        # Prefilling stage
        ...
        for _ in range(1, args.gen_length):
            with torch.no_grad():
                outputs = model(input_ids, use_cache=True, past_key_values=past_key_values)
                logits = outputs.logits[:, -1, :]
                past_key_values = outputs.past_key_values
    ...
    if args.save_path:
        ...
        file_name = f"{args.mode}_{args.gen_length}_{args.iter}_sparsity{args.sparsity}.json"
        file_path = os.path.join(args.save_path, file_name)
        res = {
            'Avg Decoding Time': avg_time,
            'Avg Prefilling Time': avg_prefilling_time,
            'Avg context length': avg_context_length,
            'Mode': args.mode,
